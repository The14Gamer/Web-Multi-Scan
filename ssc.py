...
#
# Author     :2103270692
# Tool       : SSCyberScan v1.0
# Usage      : python3 sscyberscan.py example.com
...
# SSCyberScan Help Context
def helper():
        print(bcolors.OKBLUE+"Information:"+bcolors.ENDC)
        print("------------")
        print("\t./sscyberscan.py example.com: Scans the domain example.com.")
        print("\t./sscyberscan.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
        print("\t./sscyberscan.py example.com --nospinner: Disable the idle loader/spinner.")
        print("\t./sscyberscan.py --update   : Updates the scanner to the latest version.")
        print("\t./sscyberscan.py --help     : Displays this help context.")
        print(bcolors.OKBLUE+"Interactive:"+bcolors.ENDC)
        print("------------")
        print("\tCtrl+C: Skips current test.")
        print("\tCtrl+Z: Quits SSCyberScan.")
...
def logo():
    ...
    logo_ascii = """
                     ...
                     """+bcolors.ENDC+"""(The Multi-Tool Web Vulnerability Scanner)
                     			Sin: 2103270692 (Friday Simwanza)

              """+bcolors.BG_LOW_TXT+"""Advisor = Sir Simuntala Samuel"""+bcolors.ENDC+""" for simulating threat hunting
    """
    print(logo_ascii)
    print(bcolors.ENDC)
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"SSCyberScan received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/sscyberscan_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/sscyberscan_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/sscyberscan_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/sscyberscan_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/sscyberscan_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o sscyberscan_temp_wapiti"],
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("SSCyberScan is updating....Please wait.\n")
    spinner.start()
    ...
    if rs_internet_availability == 0:
        ...
        spinner.stop()
        sys.exit(1)
    cmd = 'sha1sum sscyberscan.py | grep .... | cut -c 1-40'
    oldversion_hash = subprocess.check_output(cmd, shell=True)
    oldversion_hash = oldversion_hash.strip()
    os.system('wget -N https://raw.githubusercontent.com/skavngr/rapidscan/master/sscyberscan.py -O sscyberscan.py > /dev/null 2>&1')
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"You already have the latest version of SSCyberScan." + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"SSCyberScan successfully updated to the latest version." +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    os.system('rm /tmp/sscyberscan* > /dev/null 2>&1') # Clearing previous scan files
    os.system('clear')
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"SSCyberScan was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by SSCyberScan."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. SSCyberScan will still perform the rest of the tests. Install these tools to fully utilize the functionality of SSCyberScan."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/sscyberscan_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit SSCyberScan.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    vulreport = "ss.vul.%s.%s" % (target, date)
    ...
    if len(rs_vul_list)==0:
        ...
        with open(vulreport, "a") as report:
            while(rs_vul < len(rs_vul_list)):
                ...
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/sscyberscan_temp_"+vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+vulreport+bcolors.ENDC+" is available under the same directory SSCyberScan resides.")
    ...
    for file_index, file_name in enumerate(tool_names):
        with open(debuglog, "a") as report:
            try:
                with open("/tmp/sscyberscan_temp_"+file_name[arg1], 'r') as temp_report:
                        data = temp_report.read()
    ...
    os.system('rm /tmp/sscyberscan_te* > /dev/null 2>&1') # Clearing previous scan files
